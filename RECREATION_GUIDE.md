...
## Step 10: Configure ESLint with Comprehensive Rules

Install additional ESLint plugins for better code quality:

```bash
pnpm add -D typescript-eslint @tanstack/eslint-plugin-query eslint-config-prettier eslint-import-resolver-typescript eslint-plugin-import-x eslint-plugin-promise eslint-plugin-react eslint-plugin-react-hooks eslint-plugin-react-refresh eslint-plugin-simple-import-sort eslint-plugin-unused-imports globals
````

Update `eslint.config.mjs` with comprehensive rules following Next.js 16 pattern:
...
> **Note:** This configuration follows the Next.js 16 official pattern while adding comprehensive linting rules for TypeScript, React, imports, and code quality.

## Step 11: Upgrade to Next.js 16 (Optional)
